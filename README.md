...
- 左侧：窗口置顶、侧栏开关（⌘B）、一键打开文件夹、批量 AI 整理 / 归档 / 清理空项目。点击项目先响应侧栏选中态，内容区、主题与右侧栏下一拍再跟进，避免终端挂载卡住点击反馈；当前打开项目使用 macOS 27 Liquid Glass 选中态并带有点击动态效果与淡投影，非强调玻璃按模式统一使用深色黑色 6%、明亮白色 6% tint；玻璃按钮与选中条目使用更大圆角。
...
- 主内容区当前 Tab 使用 macOS 27 Liquid Glass 选中态，并带有点击/悬停动态反馈与更大圆角；非当前 Tab 保持轻量平面样式。
...
- 顶部面板 Tabs 与 Project 的代码编辑器 / AI 工具打开按钮使用 macOS 27 液态玻璃，圆角比早期版本更大；Tabs 整组共享玻璃材质，当前项保持清晰的选中填充，并在窄栏中自动收缩标签。
