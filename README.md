...
- **音效支持（命令运行结束 / 失败、Agent 完成，可在设置中关闭）**：新增 `kero/SoundEffects.swift` 音效播放服务，使用 macOS 内建系统音效（`/System/Library/Sounds`，随系统音量输出，不额外打包音频资源）：
  - **命令运行结束 / 失败**：终端命令经 zsh 集成上报 OSC 133;D 完成状态时按退出码播放提示音——`exitCode == 0` 播放 Glass（成功），非零播放 Basso（失败，含 Ctrl+C 中断的 130 等）。
  - **Agent 完成**：`AgentWatcher` 检测到 Agent 由 `working` 转为 `done` 时播放 Tink（无论终端是否可见）。
  - **设置开关**：`Settings → General` 新增「Sound Effects」分区——总开关 + 三个事件独立开关（Command succeeded / Command failed / Agent completed；总开关关闭时子项置灰），每行带喇叭试听按钮（无视开关直接播放，便于对比）；配置写入 `config.toml` 的 `ui.sound-effects` / `ui.sound-command-succeeded` / `ui.sound-command-failed` / `ui.sound-agent-completed`（默认全部开启，仅在关闭时写回）。同一事件 0.4s 防抖，避免大量命令同时结束时音效叠爆。
