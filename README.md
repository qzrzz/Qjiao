...
- **自定义 Git 仓库路径解析、持久化与 Pipe 读写鲁棒性修复**：修复指定自定义 Git 仓库路径（`customGitPath`）后 Git 面板频发错误以及重启应用后配置失效的问题：① **启动/恢复持久化**：修复 `TerminalManager` 在应用重启从会话快照（`restoreFromSnapshot`）和磁盘列表（`syncProjectsFromGlobalDiskState`）恢复项目对象时漏赋值 `customGitPath` 导致后续触发 `saveConfig()` 将 `nil` 写回磁盘擦除配置的问题；② **Symlink 路径规范化**：在 `Project.swift` 与 `GitScanner.swift` 中为仓库路径处理添加 `resolvingSymlinksInPath()` 规范化，解决 macOS 上由于路径符号链接（如 `/var` 与 `/private/var`）导致 `GitScanner.resolveRepositoryRoot` 与预期路径比对不一致引起的校验失败；③ **POSIX Pipe 安全读取**：在 `GitScanner`、`GitCommitEditor` 及 `DiffViewerView` 中将过期的 Foundation `readDataToEndOfFile()`（进程结束或管道关闭时在 macOS 上抛出 `NSFileHandleOperationException: Bad file descriptor`）替换为 `try? readToEnd()`，避免管道读取异常中断程序执行。
