...
- **统一子进程执行器 SubprocessRunner（根治 Bad file descriptor / 线程与 fd 泄漏）**：新增 `kero/SubprocessRunner.swift` 作为全应用唯一的 Process 启动 / 回收路径，GitScanner / SystemCommandRunner / LocalAIProcessRunner / LocalAIExecutableLocator / VendorBinLocator / ImageProcessRunner / FileViewerView 全部收敛至此，并全库清零 `readDataToEndOfFile`（统一 `try? readToEnd()`）：
  ...
  - **独立 EOF stdin pipe**：沿用不碰共享 `FileHandle.nullDevice` 的既有修复，统一提供 `makeEOFStdinPipe`。
  - **启动失败诊断统一**：domain / code / underlying 完整文案由执行器生成，GitScanner 原有「Command / Directory」上下文行保留，错误展示不变。
  - **配套加固**：`zsh -ic` 格式化器探测（`FileViewerView.interactiveShellNodeDirectory`）增加 20s 超时（原先用户 zshrc 挂起会永久阻塞）；`AIToolRegistry` 回退 `open` 与 `SidebarProbe.createGitTag` 补 `waitUntilExit`（消灭不 wait 的僵尸进程）。
...
- **设置面板左侧栏底部应用卡片交互优化**：为版本号检查更新按钮和前往官网（地球图标）按钮添加灵敏的 Hover 悬浮高亮效果（移除 `.disabled` 截断，增强微亮背景与前景色 hover 动画），接入 `macTooltip` 顶部独立浮层原生 Tooltip（显示「检查更新…」与「Qjiao 官网」提示），并将前往网页的目标 URL 更新为官方网站 `https://qzrzz.github.io/Qjiao`。
...
- **修复 Git 面板 Retry 点了无效 / Bad file descriptor 无法恢复**：① **强制介入**：Retry 不再因 2s 轮询占用的 `isRefreshing` 静默 return，先 `invalidateStatusRefresh` 作废进行中扫描再恢复；② **恢复扫描绕过 fsmonitor**：`loadStatus(recovery: true)` 全程 `git -c core.fsmonitor=false`，不依赖 daemon 是否成功 start；③ **自动自愈**：普通扫描若返回 Bad file descriptor / EBADF，扫描侧自动 recover + 以 bypass 再扫一次；④ **IPC 清理放宽**：`recoverFilesystemMonitor` 始终 stop + 删 socket（rev-parse 失败时回退 `.git/` 或 worktree `gitdir:` 文件），仅启用了内建 daemon 时才 start；⑤ **Process stdin**：每次打开独立 `/dev/null`，避免共享 `FileHandle.nullDevice` 被 Process 关闭后后续启动稳定 EBADF，启动失败时再重试一次。
- **修复 Git 面板 Retry recovery busy 竞态与操作 spinner 卡住**：① recovery 用 `recoveryID` / `isRecovering` 独立记账，`clearRepositoryState`（cwd 切换）可安全作废 in-flight recovery，不再被 `isBusy = runningOperationID != nil` 误清或卡死；② Retry 按钮在恢复中显示 ProgressView +「Recovering…」；③ 操作按钮经 `beginGitAction`：model 早退（校验失败 / failImmediately）且未进入 busy 时立即清除 `activeActionTarget`，避免 showsProgress 永久转圈；④ EBADF 自愈后 60s 内默认 bypass fsmonitor，避免 2s 轮询反复 stop/start daemon thrash。
- **加固 Git 面板 Retry 真正可恢复 + 详细错误诊断（Bad file descriptor）**：针对「Retry 仍无效、只有重启 App 才恢复」的残留路径：① **git 子进程超时**（常规 45s / 恢复 20s，超时 SIGTERM→SIGKILL），避免坏 fsmonitor socket 上 `git status` 永久阻塞把 actor 队列卡死；② **Retry 扫描绕开 actor 串行队列**（`loadStatusForRecovery`），不再排在可能已挂死的常规扫描后面；③ **允许再次点击抢占**卡住的 recovery（换新 `recoveryID`），恢复中 UI 显示「Recovering Git Status…」；④ **stdin 改用独立 EOF pipe**（彻底不用 `FileHandle.nullDevice`），启动失败再以 `git -C`（不设 process cwd）重试；⑤ **错误信息扩展**：命令名、exit code、目录、Context、Launch domain/code、已执行的 Recovery steps，错误区可滚动选中复制。
- **彻底消除 LocalAI / 子进程 `Failed to launch AI CLI: Bad file descriptor` 错误**：将 `LocalAIProcessRunner`、`LocalAIExecutableLocator`、`ImageProcessRunner`、`VendorBinLocator`、`FileViewerView`、`SidebarProbe` 中所有传入 `process.standardInput` / `process.standardError` 的 `FileHandle.nullDevice` 彻底替换为独立的 EOF `Pipe()`。解决 macOS Foundation `Process` 退出清理句柄时关闭 `FileHandle.nullDevice` 共享单例的底层 fd，导致后续子进程启动崩溃 `POSIX error 9 (EBADF)` 的系统级兼容性漏洞。
