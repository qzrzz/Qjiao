...
/// 自动化协议使用的受限 JSON 值模型，避免把任意 Foundation 对象暴露到
/// 本地 socket 边界。
enum QjiaoJSONValue: Codable, Equatable, Sendable {
    case string(String)
    case number(Double)
    case bool(Bool)
    case object([String: QjiaoJSONValue])
    case array([QjiaoJSONValue])
    case null
    ...
    init(from decoder: Decoder) throws {
        ...
        if container.decodeNil() {
            ...
            self = .string(value)
        } else if let value = try? container.decode([String: QjiaoJSONValue].self) {
            self = .object(value)
        } else if let value = try? container.decode([QjiaoJSONValue].self) {
            self = .array(value)
        } else {
    ...
    var arrayValue: [QjiaoJSONValue]? {
        guard case .array(let value) = self else { return nil }
        return value
    }

    var objectValue: [String: QjiaoJSONValue]? {
        guard case .object(let value) = self else { return nil }
        return value
    }
}

struct QjiaoAutomationRequest: Codable, Sendable {
    let version: Int
    let id: String
    let method: String
    let token: String
    let terminalID: String
    let params: [String: QjiaoJSONValue]
}

struct QjiaoAutomationErrorPayload: Codable, Sendable {
    let code: String
    let message: String
}

struct QjiaoAutomationResponse: Codable, Sendable {
    let version: Int
    let id: String
    let ok: Bool
    let result: QjiaoJSONValue?
    let error: QjiaoAutomationErrorPayload?

    static func success(id: String, result: QjiaoJSONValue) -> Self {
    ...
    static func failure(id: String, code: String, message: String) -> Self {
        Self(
            ...
            ok: false,
            result: nil,
            error: QjiaoAutomationErrorPayload(code: code, message: message)
        )
    }
}

enum QjiaoAutomationWireError: Error, CustomStringConvertible {
    case message(String)
...
/// 一个连接承载一条 NDJSON 请求和一条响应。socket 只绑定在本机，且由
/// QjiaoCLIService 为每个终端签发 capability，因此不依赖宽泛的进程级 token。
final class QjiaoAutomationSocketServer: @unchecked Sendable {
    typealias Handler = @Sendable (
        QjiaoAutomationRequest,
        @escaping @Sendable (QjiaoAutomationResponse) -> Void
    ) -> Void
    ...
    private let queue = DispatchQueue(label: "sh.qjiao.automation.socket")
    private let workers = DispatchQueue(
        label: "sh.qjiao.automation.clients",
        qos: .userInitiated,
    ...
    init(path: String, handler: @escaping Handler) throws {
        ...
        guard listener >= 0 else {
            throw QjiaoAutomationWireError.message(
        ...
        while true {
            ...
            if client < 0 {
                ...
                if errno != EAGAIN && errno != EWOULDBLOCK {
                    NSLog("qjiao: automation accept failed: %s", strerror(errno))
                }
                return
            }
            do {
    ...
    private func readRequest(from client: Int32) {
        ...
        do {
            request = try JSONDecoder().decode(QjiaoAutomationRequest.self, from: data)
        } catch {
            respond(
                .failure(
    ...
    private func respond(_ response: QjiaoAutomationResponse, to client: Int32) {
        defer { Darwin.close(client) }
        guard var data = try? JSONEncoder.qjiaoAutomation.encode(response) else { return }
        data.append(0x0A)
    ...
    static func exchange(
        path: String,
        request: QjiaoAutomationRequest,
        timeout: TimeInterval = 5
    ) throws -> QjiaoAutomationResponse {
        ...
        guard connected == 0 else {
            throw QjiaoAutomationWireError.message(
                "Could not connect to Qjiao automation: \(String(cString: strerror(errno)))."
            )
        }

        var encoded = try JSONEncoder.qjiaoAutomation.encode(request)
        encoded.append(0x0A)
        ...
        while responseData.count <= maximumMessageBytes {
            ...
            if count > 0 {
                ...
                throw QjiaoAutomationWireError.message(
                    "Could not read Qjiao's automation response: \(String(cString: strerror(errno)))."
                )
            }
        }
        guard responseData.count <= maximumMessageBytes else {
            throw QjiaoAutomationWireError.message(
                "Qjiao returned an oversized automation response."
            )
        }
        ...
        guard !responseData.isEmpty else {
            throw QjiaoAutomationWireError.message(
                "Qjiao closed the automation connection without a response."
            )
        }
        return try JSONDecoder().decode(QjiaoAutomationResponse.self, from: responseData)
    }
...
extension JSONEncoder {
    static var qjiaoAutomation: JSONEncoder {
