...
/// 已认证 socket 请求的主线程路由器。capability 先解析到一个终端，再将
/// 所有目标限制在该终端所属项目内，不能通过猜 UUID 操作其它窗口。
@MainActor
enum QjiaoAutomationRouter {
    ...
    static func route(
        ...
        case "agent.list":
            let agents = projectContexts(caller.project, manager: caller.manager)
                .compactMap { context -> QjiaoJSONValue? in
                    guard context.session?.effectiveAutomationAgentStatus() != nil else {
                        return nil
                    }
    ...
    private static func runInPane(
        ...
        guard let argv = stringArray(request.params["argv"]),
              ...
              !argv[0].isEmpty,
              argv.allSatisfy({ $0.utf8.count <= 16_384 && isSafeShellArgument($0) })
        else {
            return failure(
                request, "invalid_params",
    ...
    private static func readPane(
        ...
        do {
            let text = try session.automationReadText(
                maxLines: lines,
                maxColumns: columns,
        ...
        } catch QjiaoAutomationReadError.agentNotIdle {
            return failure(
                request,
                "agent_not_idle",
                "The target Agent is still working or blocked; wait for done before reading history."
            )
        } catch {
            return failure(request, "read_failed", "Qjiao could not read the terminal text.")
        }
    }
    ...
    private static func startAgent(
        ...
        guard session.effectiveAutomationAgentStatus() == nil else {
            return failure(
        ...
        guard let kindName = request.params["kind"]?.stringValue,
              let kind = agentKind(from: kindName) else {
            return failure(
                request, "invalid_agent_kind",
                "Supported kinds: \(supportedAgentKinds.joined(separator: ", "))."
            )
        }
        let duplicate = caller.project.sessions.contains {
            $0.id != session.id && $0.automationAgent?.alias == alias
        }
        guard !duplicate else {
            return failure(request, "alias_in_use", "Another Agent in this project already uses alias \(alias).")
        }
        let extra = stringArray(request.params["argv"]) ?? []
        guard extra.count <= 128,
              extra.allSatisfy({ $0.utf8.count <= 16_384 && isSafeShellArgument($0) }) else {
            return failure(
                request, "invalid_params",
        ...
        let command = ([agentExecutable(for: kind)] + extra).map(shellQuote).joined(separator: " ")
    ...
    private static func promptAgent(
        ...
        guard let target = targetAgent(request, caller: caller),
              ...
              let status = session.effectiveAutomationAgentStatus() else {
            return failure(request, "agent_not_found", "No matching Agent exists in this project.")
        }
        guard status.phase == .created
            ...
            return failure(
                ...
                "\(status.alias ?? status.kind.displayName) is \(status.phase.rawValue); guarded prompts are disabled while blocked. Use +pane send for explicit raw input."
            )
        }
        guard session.isAutomationAgentRunning(kind: status.kind) else {
            return failure(
                request, "agent_not_running",
                "The Agent process is no longer running; start it again before sending a guarded prompt."
            )
        }
    ...
    private static func reportAgent(
        ...
        guard let name = request.params["state"]?.stringValue,
              let phase = QjiaoAgentPhase(rawValue: name),
              phase != .created else {
            return failure(
                request, "invalid_params",
    ...
    private static func targetAgent(
        ...
        if request.params["pane_id"] != nil {
            guard let pane = targetPane(request, caller: caller),
                  pane.session?.effectiveAutomationAgentStatus() != nil else { return nil }
            return pane
        }
        if let alias = request.params["alias"]?.stringValue {
            return projectContexts(caller.project, manager: caller.manager).first {
                $0.session?.effectiveAutomationAgentStatus()?.alias == alias
            }
        }
        return caller.session?.effectiveAutomationAgentStatus() == nil ? nil : caller
    }
    ...
    private static func paneSnapshot(
        ...
        var object: [String: QjiaoJSONValue] = [
        ...
        if let session = context.session {
            ...
            object["agent"] = session.effectiveAutomationAgentStatus().map(agentSnapshot) ?? .null
        }
        return .object(object)
    }

    private static func agentSnapshot(_ status: QjiaoAgentStatus) -> QjiaoJSONValue {
        .object([
            "alias": status.alias.map(QjiaoJSONValue.string) ?? .null,
            "kind": .string(status.kind.rawValue),
            "state": .string(status.phase.publicPhase.rawValue),
    ...
    private static let supportedAgentKinds = AgentKind.allCases.map(\.rawValue)

    private static func agentKind(from value: String) -> AgentKind? {
        if let exact = AgentKind(rawValue: value) { return exact }
        return AgentKind.parse(processName: value)
    }

    private static func agentExecutable(for kind: AgentKind) -> String {
        switch kind {
    ...
    private static func stringArray(_ value: QjiaoJSONValue?) -> [String]? {
